...
class Game:
    def __init__(self, console):
        ...
        R.active_map = self.map = [[0 for _ in xrange(R.SCREEN_HEIGHT)] for _ in xrange(R.SCREEN_WIDTH)]
    ...
    def update(self):
        print "updating", self.you.x, self.you.y
        time.sleep(1)
...
def scrolling_map(p, hs, s, m):
    ...
    Get the position of the camera in a scrolling map:
        ...
        :param p: is the position of the player.
        :param hs: is half of the screen size
        :param s: is the full screen size.
        :param m: is the size of the map.
    """
    if p < hs:
        return 0
    elif p > m - hs:
