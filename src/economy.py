...
P_DIF_ADJ = 3  # When offer is accepted and exceeds what we thought the price was, adjust it upward by this amount
N_DIF_ADJ = 3  # When offer is accepted & is lower than we thought the price was, adjust it downward by this amount
P_DIF_THRESH = 2.5  # Threshold at which adjustment of P_DIF_ADJ is added to the perceived value of a commodity
N_DIF_THRESH = 2.5  # Threshold at which adjustment of P_DIF_ADJ is subtracted from the perceived value of a commodity
...
def setup_resources():
    ...
    ore_app = {'mountain': 0, 'tundra': 5, 'taiga': 8, 'temperate forest': 10, 'temperate steppe': 10, 'rain forest': 5,
               'tree savanna': 12, 'grass savanna': 15, 'dry steppe': 20,
               'semi-arid desert': 20, 'arid desert': 20, 'river': 0}
    food_app = {'mountain': 0, 'tundra': 0, 'taiga': 0, 'temperate forest': 1, 'temperate steppe': 5, 'rain forest': 0,
                'tree savanna': 12, 'grass savanna': 20, 'dry steppe': 20,
                'semi-arid desert': 0, 'arid desert': 0, 'river': 5}
    clay_app = {'mountain': 0, 'tundra': 0, 'taiga': 0, 'temperate forest': 0, 'temperate steppe': 0, 'rain forest': 10,
                'tree savanna': 0, 'grass savanna': 10, 'dry steppe': 20,
                'semi-arid desert': 20, 'arid desert': 20, 'river': 5}
    silt_app = {'mountain': 0, 'tundra': 0, 'taiga': 0, 'temperate forest': 0, 'temperate steppe': 0, 'rain forest': 0,
                'tree savanna': 0, 'grass savanna': 0, 'dry steppe': 0,
                'semi-arid desert': 0, 'arid desert': 0, 'river': 500}
    wood_app = {'mountain': 0, 'tundra': 0, 'taiga': 1000, 'temperate forest': 1000, 'temperate steppe': 0,
                'rain forest': 1000, 'tree savanna': 1000, 'grass savanna': 0, 'dry steppe': 0,
                'semi-arid desert': 0, 'arid desert': 0, 'river': 0}
    flax_app = {'mountain': 0, 'tundra': 0, 'taiga': 0, 'temperate forest': 0, 'temperate steppe': 10, 'rain forest': 0,
                'tree savanna': 0, 'grass savanna': 20, 'dry steppe': 20,
    ...
    bronze = Resource(name='bronze', category='ores', resource_class='strategic', gather_amount=4, break_chance=100,
                      app_chances=ore_app, app_amt=amt)
    iron = Resource(name='iron', category='ores', resource_class='strategic', gather_amount=3, break_chance=60,
                    app_chances=ore_app, app_amt=amt)
    food = Resource(name='food', category='foods', resource_class='strategic', gather_amount=8, break_chance=1,
                    app_chances=food_app, app_amt=amt)
    clay = Resource(name='clay', category='clays', resource_class='strategic', gather_amount=4, break_chance=280,
                    app_chances=clay_app, app_amt=amt)
    silt = Resource(name='silt', category='clays', resource_class='strategic', gather_amount=4, break_chance=350,
                    app_chances=silt_app, app_amt=amt)
    wood = Resource(name='wood', category='woods', resource_class='strategic', gather_amount=4, break_chance=250,
                    app_chances=wood_app, app_amt=amt)
    oak = Resource(name='oak', category='woods', resource_class='strategic', gather_amount=2, break_chance=180,
                   app_chances=wood_app, app_amt=amt)
    flax = Resource(name='flax', category='cloths', resource_class='strategic', gather_amount=4, break_chance=250,
                    app_chances=flax_app, app_amt=amt)

    RESOURCES = [copper, bronze, iron, clay, silt, wood, oak, flax, food]
    RESOURCE_TYPES = {}
    STRATEGIC_TYPES = {}
    ##
    COMMODITY_TYPES = {}
    ...
    for resource in RESOURCES:
        ...
        if resource.category not in RESOURCE_TYPES:
            RESOURCE_TYPES[resource.category] = [resource]
        else:
            RESOURCE_TYPES[resource.category].append(resource)

        if resource.category not in COMMODITY_TYPES.keys():
            COMMODITY_TYPES[resource.category] = [resource]
        else:
            COMMODITY_TYPES[resource.category].append(resource)

        if resource.resource_class == 'strategic':
            if not resource.resource_class in STRATEGIC_TYPES.keys():
                STRATEGIC_TYPES[resource.category] = [resource]
            else:
    ...
    GOODS = [copper_tools, bronze_tools, iron_tools, silt_pottery, clay_pottery, wood_furniture, oak_furniture,
             flax_clothing,
    ...
    gatherers_by_token = {
        'food': {'name': 'Food Farmer', 'consumed': [], 'essential': ['tools'],
        ...
        'oak': {'name': 'Woodcutter', 'consumed': [], 'essential': ['tools'], 'preferred': ['clothing']}
    }
    # Good producers
    producers_by_token = {
        'copper tools': {'name': 'Copper Blacksmith', 'finished_goods': 'copper tools', 'consumed': ['woods'],
                         'essential': [], 'preferred': ['clothing']},
        'bronze tools': {'name': 'Bronze Blacksmith', 'finished_goods': 'bronze tools', 'consumed': ['woods'],
                         'essential': [], 'preferred': ['clothing']},
        'iron tools': {'name': 'Iron Blacksmith', 'finished_goods': 'iron tools', 'consumed': ['woods'],
                       'essential': [], 'preferred': ['clothing']},
        'silt pottery': {'name': 'Silt Potter', 'finished_goods': 'silt pottery', 'consumed': [], 'essential': [],
                         'preferred': ['clothing']},
        'clay pottery': {'name': 'Clay Potter', 'finished_goods': 'clay pottery', 'consumed': [], 'essential': [],
                         'preferred': ['clothing']},
        'wood furniture': {'name': 'Wood Carptenter', 'finished_goods': 'wood furniture', 'consumed': [],
                           'essential': ['tools'], 'preferred': ['clothing']},
        'oak furniture': {'name': 'Wood Carptenter', 'finished_goods': 'oak furniture', 'consumed': [],
                          'essential': ['tools'], 'preferred': ['clothing']},
        'flax clothing': {'name': 'Flax Clothier', 'finished_goods': 'flax clothing', 'consumed': [],
                          'essential': ['tools'], 'preferred': []},
        'iron sword': {'name': 'Iron Weaponsmith', 'finished_goods': 'iron sword', 'consumed': ['woods'],
                       'essential': ['tools'], 'preferred': ['clothing']},
        'copper sword': {'name': 'Copper Weaponsmith', 'finished_goods': 'copper sword', 'consumed': ['woods'],
                         'essential': ['tools'], 'preferred': ['clothing']},
    }
...
class TradeHouse:
    ...
    def collect_info(self, commodity="none"):
        ...
        demand, demand_cost = self.find_averages("buys", commodity)
        supply, supply_cost = self.find_averages("asks", commodity)
    ...
    def find_averages(self, transaction_type, commodity):
        amount = 0.0
        amount_cost = 0.0
    ...
    def resolve_offers_city(self, commodity, city):
        ...
        self.trades["buys"][commodity] = sorted(self.trades["buys"][commodity], key=lambda offer: offer.price,
                                                reverse=True)
        city.trade_house.trades["asks"][commodity] = sorted(city.trade_house.trades["asks"][commodity],
        ...
        while len(self.trades["buys"][commodity]) > 0 and len(city.trade_house.trades["asks"][commodity]) > 0:
            ...
            seller = city.trade_house.trades["asks"][commodity][0]  # sell offer
            ...
            self.check_for_time_out(city.trade_house.trades["asks"][commodity], self.trades["buys"][commodity], self,
                                    city)
...
class Resource:
    def __init__(self, name="", category="", resource_class="", gather_amount=0, break_chance=0, app_chances=(),
                 app_amt=0, variant="none", quantity=0.0):
        self.name = name
...
class Cargo(Resource):
    def __init__(self, variant="none", quantity=0.0):
...
class Agent(object):
    ...
    def check_for_needed_items(self):
        ...
        critical_items = [type_of_item for type_of_item in self.consumed + self.essential if
                          not self.has_token(type_of_item)]
    ...
    def place_bid(self, token_to_bid):
        ...
        self.economy.auctions[token_to_bid].bids.append(
            Offer(owner=self, commodity=token_to_bid, price=bid_price, quantity=quantity))
    ...
    def create_sell(self, sell_item, prod_adj_amt):
        ...
        if quantity_to_sell > 0:
            self.last_turn.append(
                'Offered to sell ' + str(quantity_to_sell) + ' ' + sell_item + ' for ' + str(sell_price))
            self.last_turn.append('Prod: ' + str(production_cost) + '; min: ' + str(min_sale_price))
            self.economy.auctions[sell_item].sells.append(
                Offer(owner=self, commodity=sell_item, price=sell_price, quantity=quantity_to_sell))
        else:
    ...
    def eval_sell_rejected(self, type_of_item):
        ...
        if self.economy.auctions[type_of_item].demand:
            ...
            self.perceived_values[type_of_item].center = self.perceived_values[
                                                             type_of_item].center - ASK_REJECTED_ADJUSTMENT
            self.perceived_values[type_of_item].uncertainty += REJECTED_UNCERTAINTY_AMOUNT
...
class ResourceGatherer(Agent):
    ...
    def take_turn(self):
        ...
        self.eval_need()
        self.pay_taxes()
        self.create_sell(sell_item=self.resource,
                         prod_adj_amt=self.gather_amount)  # <- will check to make sure we have items to sell...
        self.turns_alive += 1
    ...
    def check_production_cost(self):
        ...
        for type_of_item in self.essential + self.preferred:
            for token_of_item in self.economy.available_types[type_of_item]:
                if token_of_item in self.inventory:
                    production_cost += int(round(self.economy.auctions[token_of_item].mean_price * (
                        COMMODITY_TOKENS[token_of_item].break_chance / 1000)))
                    break
    ...
    def gather_resources(self):
        ...
        if amount > 0:
            ...
            for type_of_item in self.preferred + self.essential:
                for token_of_item in self.economy.available_types[type_of_item]:
                    if token_of_item in self.inventory and roll(1, 1000) <= COMMODITY_TOKENS[
                        token_of_item].break_chance:
                        self.inventory.remove(token_of_item)
                        break
    ...
    def eval_sell_rejected(self, type_of_item):
        if self.economy.auctions[type_of_item].demand:
            ...
            self.perceived_values[type_of_item].center = max(
                self.perceived_values[type_of_item].center - ASK_REJECTED_ADJUSTMENT,
                min_sale_price + self.perceived_values[type_of_item].uncertainty)
...
class GoodsProducer(Agent):
    def __init__(self, name, economy, finished_goods, consumed, essential, preferred):
        ...
        self.perceived_values = {finished_goods.name: Value(START_VAL, START_UNCERT),
                                 self.input: Value(START_VAL, START_UNCERT)}
        for type_of_item in consumed + essential + preferred:
            for token_of_item in COMMODITY_TYPES[type_of_item]:
                self.perceived_values[token_of_item.name] = Value(START_VAL, START_UNCERT)
            for token_of_item in COMMODITY_TYPES['foods']:
    ...
    def take_turn(self):
        ...
        self.eval_need()
        self.pay_taxes()
        self.create_sell(sell_item=self.finished_goods.name,
                         prod_adj_amt=self.out_amt)  # <- will check to make sure we have items to sell...
        self.turns_alive += 1
...
class Merchant(object):
    def __init__(self, name, home_economy, traded_item, consumed, essential, preferred, attached):
        self.sell_economy = None
        self.buy_economy = None
        self.turns_alive = 0
        self.buys = 0
        self.sells = 0
    ...
    def consume_food(self):
        ...
        # # Bid on food if we have less than a certain stockpile
        if self.inventory.count('food') < FOOD_BID_THRESH:
            self.place_bid(economy=self.current_location,
    ...
    def increment_cycle(self):
        ...
        if (self.current_location == self.buy_economy and len(self.inventory) > self.INVENTORY_SIZE - 2) or \
            ...
            if self.current_location.owner:
                ...
                if self.current_location == self.buy_economy:
                    self.current_location = self.sell_economy
                elif self.current_location == self.sell_economy:
    ...
    def place_bid(self, economy, token_to_bid):
        ...
        if token_to_bid == self.traded_item:
            quantity = self.INVENTORY_SIZE - (len(self.inventory) + 1)
        else:
            quantity = roll(1, 2)
        # print self.name, 'bidding on', quantity, token_to_bid, 'for', bid_price, 'at',
        # self.current_location.owner.name
        # print self.name, 'bidding for', quantity, token_to_bid
        if quantity > 0:
            self.last_turn.append('Bid on ' + str(quantity) + ' ' + token_to_bid + ' for ' + str(bid_price))
            economy.auctions[token_to_bid].bids.append(
                Offer(owner=self, commodity=token_to_bid, price=bid_price, quantity=quantity))
        else:
    ...
    def create_sell(self, economy, sell_item):
        ...
        # print self.name, 'selling', quantity_to_sell, sell_item
        if quantity_to_sell > 0:
            # print self.name, 'selling', quantity_to_sell, sell_item, 'for', sell_price, 'at',
            # self.current_location.owner.name
            self.last_turn.append(
                'Offered to sell ' + str(quantity_to_sell) + ' ' + sell_item + ' for ' + str(sell_price))
            economy.auctions[sell_item].sells.append(
                Offer(owner=self, commodity=sell_item, price=sell_price, quantity=quantity_to_sell))
        else:
        ...
        if self.current_location == self.buy_economy:
            ...
            if price > our_mean * P_DIF_THRESH:
                ...
                self.buy_perceived_values[type_of_item].center = \
                    max(self.buy_perceived_values[type_of_item].center - N_DIF_ADJ,
                        self.buy_economy.local_taxes + self.buy_perceived_values[type_of_item].uncertainty)
        ...
        if self.current_location.auctions[type_of_item].supply:
            if self.current_location == self.buy_economy:
                if price is None:
                    ...
                    # Radical re-evaluation of the price
                    self.buy_perceived_values[type_of_item].center = price + self.buy_perceived_values[
                        type_of_item].uncertainty
                    self.buy_perceived_values[type_of_item].uncertainty += REJECTED_UNCERTAINTY_AMOUNT
                ...
                if price == None:
                    ...
                    # Radical re-evaluation of the price
                    self.sell_perceived_values[type_of_item].center = price + self.sell_perceived_values[
                        type_of_item].uncertainty
                    self.sell_perceived_values[type_of_item].uncertainty += REJECTED_UNCERTAINTY_AMOUNT
    ...
    def check_min_sell_price(self):
        ...
        # These items are required for production, but not used. Find the item's cost *
        # (break chance/1000) to find avg cost
        for type_of_item in self.consumed + self.essential + self.preferred:
            for token_of_item in COMMODITY_TYPES[type_of_item]:
                if token_of_item in self.inventory:
                    production_cost += int(round(self.buy_economy.auctions[token_of_item].mean_price * (
                        COMMODITY_TOKENS[token_of_item].break_chance / 1000)))
                    break
        # Take into account the taxes we pay
    ...
    def eval_sell_rejected(self, type_of_item):
        ...
        if self.current_location.auctions[type_of_item].demand:
            ...
            if self.current_location == self.buy_economy:
                # self.perceived_values[type_of_item].center = max(self.perceived_values[type_of_item].center - ASK_REJECTED_ADJUSTMENT, min_sale_price + self.perceived_values[type_of_item].uncertainty)
                self.buy_perceived_values[type_of_item].center = self.buy_perceived_values[
                                                                     type_of_item].center - ASK_REJECTED_ADJUSTMENT
                self.buy_perceived_values[type_of_item].uncertainty += REJECTED_UNCERTAINTY_AMOUNT
            elif self.current_location == self.sell_economy:
                self.sell_perceived_values[type_of_item].center = self.sell_perceived_values[
                                                                      type_of_item].center - ASK_REJECTED_ADJUSTMENT
                self.sell_perceived_values[type_of_item].uncertainty += REJECTED_UNCERTAINTY_AMOUNT
...
class Auction:
    # Separate "auction" for each commodity
    ...
    def update_mean_price(self):
        ...
        self.mean_price = int(
            round(sum(self.price_history[-HIST_WINDOW_SIZE:]) / len(self.price_history[-HIST_WINDOW_SIZE:])))
        ...
        for commodity, auction in self.auctions.iteritems():
            ...
            while not len(auction.bids) == 0 and not len(auction.sells) == 0:
                ...
                if buyer.price < seller.price:
                    ...
                    buyer.owner.last_turn.append(
                        'Bought ' + str(quantity) + ' ' + commodity + ' from ' + seller.owner.name + ' at ' + str(
                            price))
                    seller.owner.sells += quantity
                    seller.owner.last_turn.append(
                        'Sold ' + str(quantity) + ' ' + commodity + ' to ' + buyer.owner.name + ' at ' + str(price))
...
class CityEconomy(object):
    ...
    def add_commodity_to_economy(self, commodity):
        # sets up the auctions and available for trade in the other.
        category = COMMODITY_TOKENS[commodity].category
        if category in self.available_types.keys():
            if commodity not in self.available_types[category]:
                self.available_types[category].append(commodity)
    ...
    def run_simulation(self):
        ...
        for commodity, auction in self.auctions.iteritems():
            ...
            if len(self.prices[commodity]) > 0:
                ...
                auction.price_history.append(auction.price_history[-1])
                # print (commodity + ' was not sold this round (' + str(num_bids) +
                # ' bids, ' + str(num_sells) + ' sells)')
