...
FOOD_BID_THRESHOLD = 4  # Will bid on food when lower than this amount
...
def setup_resources():
    ...
    bronze = Resource(name='bronze', category='ores', resource_class='strategic', gather_amount=4, break_chance=100,
                      app_chances=ore_app, app_amt=amt)
    iron = Resource(name='iron', category='ores', resource_class='strategic', gather_amount=4, break_chance=60,
                    app_chances=ore_app, app_amt=amt)
    ...
    clay = Resource(name='clay', category='clays', resource_class='strategic', gather_amount=4, break_chance=200,
                    app_chances=clay_app, app_amt=amt)
    silt = Resource(name='silt', category='clays', resource_class='strategic', gather_amount=4, break_chance=200,
                    app_chances=silt_app, app_amt=amt)
    wood = Resource(name='wood', category='woods', resource_class='strategic', gather_amount=4, break_chance=200,
    ...
    flax = Resource(name='flax', category='cloths', resource_class='strategic', gather_amount=4, break_chance=200,
    ...
    for resource in RESOURCES:
        ...
        if not resource.category in RESOURCE_TYPES:
            RESOURCE_TYPES[resource.category] = [resource]
        else:
            RESOURCE_TYPES[resource.category].append(resource)

        if not resource.category in COMMODITY_TYPES.keys():
    ...
    for good in GOODS:
        ...
        if not good.category in GOOD_TYPES:
            GOOD_TYPES[good.category] = [good]
        else:
            GOOD_TYPES[good.category].append(good)

        if not good.category in COMMODITY_TYPES.keys():
            COMMODITY_TYPES[good.category] = [good]
        else:
            COMMODITY_TYPES[good.category].append(good)

        if not good.category in GOODS_BY_RESOURCE_TOKEN.keys():
            GOODS_BY_RESOURCE_TOKEN[good.material.name] = [good]
        else:
    ...
    producers_by_token = {
        ...
        'clay pottery': {'name': 'Clay Potter', 'finished_goods': 'clay pottery', 'consumed': [], 'essential': [],
                         'preferred': ['clothing']},
        'wood furniture': {'name': 'Wood Carpenter', 'finished_goods': 'wood furniture', 'consumed': [],
                           'essential': ['tools'], 'preferred': ['clothing']},
        # 'oak furniture':{'name':'Wood Carpenter', 'finished_goods':'wood furniture', 'consumed':[],
        # 'essential':['tools'], 'preferred':['clothing'] },
...
class Agent(object):
    ...
    def pay_taxes(self, economy=None):
        if economy is None:
    ...
    def check_production_cost(self):
        print "Should not be calling this base class."
...
class ResourceGatherer(Agent):
    ...
    def consume_food(self):
        ...
        if 'Farmer' in self.name and self.able_to_produce:
            ...
            if self.inventory.count('food') < FOOD_BID_THRESHOLD:
...
class GoodProducer(Agent):
    def __init__(self, name, economy, finished_good, consumed, essential, preferred):
        ...
        self.perceived_values = {finished_good.name: Value(START_VAL, START_UNCERT),
    ...
    def check_production_ability(self):
        ...
        if critical_items == [] and has_required_input and (self.inventory_size - len(self.inventory) > 0):
            self.produce_items()
            # else:
            # print self.name, '- not producing because: critical items-', critical_items, 'required_input',
            # has_required_input, 'inventory:', self.inventory
    ...
    def eval_sell_rejected(self, type_of_item):
        ...
        if self.economy.auctions[type_of_item].demand:
            ...
            # self.perceived_values[type_of_item].center = max(self.perceived_values[type_of_item].center
            #       - ASK_REJECTED_ADJUSTMENT, min_sale_price + self.perceived_values[type_of_item].uncertainty)
...
class Merchant(Agent):
    ...
    def consume_food(self):
        ...
        for token_of_item in self.current_location.available_types['foods']:
            if token_of_item in self.inventory:
                ...
                # TODO: understand wtf is going on here. food?
                if roll(1, 5) == 1:
                    self.inventory.remove(token_of_item)
                    self.turns_since_food = 0
                    break
    ...
    def place_bid(self, economy, token_to_bid):
        ...
        if self.current_location == self.buy_economy:
            ...
            # TODO: establish some kind of values for a "wild guess". CURRENT 0
            est_price = 0
            uncertainty = 0
    ...
    def eval_sell_rejected(self, type_of_item):
        ...
        if self.current_location.auctions[type_of_item].demand:
            ...
            min_sale_price = int(round(production_cost * PROFIT_MARGIN))
            if self.current_location == self.buy_economy:
                # self.perceived_values[type_of_item].center = max(self.perceived_values[
                # type_of_item].center - ASK_REJECTED_ADJUSTMENT, min_sale_price +
                # self.perceived_values[type_of_item].uncertainty)
...
class Economy:
    ...
    def add_goods_producer(self, good):
        ...
        producer = GoodProducer(name=info['name'], economy=self, finished_good=COMMODITY_TOKENS[good],
    ...
    def add_merchant(self, sell_economy, traded_item, attached_to=None):
        ...
        merchant = Merchant(name=traded_item + ' merchant', buy_economy=self, sell_economy=sell_economy,
                            traded_item=traded_item, consumed=info['consumed'], essential=info['essential'],
                            preferred=info['preferred'], attached_to=attached_to)
    ...
    def run_simulation(self):
        ...
        for commodity, auction in self.auctions.iteritems():
            ...
            while not len(auction.bids) == 0 and not len(auction.sells) == 0:
                ...
                if buyer.price < seller.price and (
                                buyer.owner.turns_alive < 10 or (
                                        commodity == 'food' and buyer.owner.turns_since_food > 2)):
                ...
                if buyer.price < seller.price:
                    ...
                    if quantity > 0:
                        ...
                        buyer.owner.last_turn.append(
                                'Bought ' + str(
                                        quantity) + ' ' + commodity + ' from ' + seller.owner.name + ' at ' + str(
                                        price))
                        seller.owner.sells += quantity
                        seller.owner.last_turn.append(
                                'Sold ' + str(quantity) + ' ' + commodity + ' to ' + buyer.owner.name + ' at ' + str(
                                        price))
