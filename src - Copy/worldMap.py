...
class Tile:
    ...
    def __init__(self, x, y, blocked, block_sight=None, char=" ", cost=10, bg=[60, 100, 80], fg=[255, 255, 255],
                 map_tile=False):
        ...
        for cell_x in range(self.w):
            for cell_y in range(self.h):
                ...
                if value > MOUNTAIN_THRESHOLD:  # mountain
                    ...
                    self.tiles[cell_x][cell_y] = Tile(cell_x, cell_y, False, map_tile=True, bg=[tote, tote, tote],
                                                      cost=40)
                    ...
                    self.tiles[cell_x][cell_y] = Tile(cell_x, cell_y, False, map_tile=True,
                                                      bg=[10, ran_1, 10 + libtcod.random_get_int(0, -5, 5)], cost=15)
                    self.tiles[cell_x][cell_y].elevation = value
                    ...
                    self.tiles[cell_x][cell_y] = Tile(cell_x, cell_y, False, map_tile=True,
                                                      bg=[value + libtcod.random_get_int(0, -5, 5),
                                                          value - test + libtcod.random_get_int(0, -5, 5), test])
                    self.tiles[cell_x][cell_y].elevation = value
...
class Dungeon(POI):
    def __init__(self, x, y, level=1):
        POI.__init__(self, x, y, libtcod.Color(libtcod.random_get_int(0, 50, 255), libtcod.random_get_int(0, 0, 100),
                                               libtcod.random_get_int(0, 100, 150)), "#")
        self.level = level
        self.exp = 0
        self.floors = []
    ...
    def generate_floors(self, level=1, num=5):
        for ID in range(num):
            ...
            for n in range(monster_num):
                ...
                temp = entities.Object(0, 0, char=mons["char"], name=mons["name"], colour=colour, blocks=False,
