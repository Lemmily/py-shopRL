...
def floor_feature(dungeon_type):
    ...
    if debug_mode:
        message('EXPLORED: ' + str(room_type) + ', dungeon type ' + str(dungeon_type), libtcod.light_grey, libtcod.light_grey)
...
class Store:
    ...
    def raise_contract(self, asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, accepted = False):
        
        new_contract = Contract(asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, accepted)
    ...
    def rescue_corpse(self, hero):
        ...
        self.raise_contract(asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, False)
...
class Mayoress:
    ...
    def __init__(self, inventory, wealth, base_level, reputation, population, alignment, order, noticeboard, personality, flags):
            ...
            if map[x][y].dungeon.population > map[x][y].dungeon.base_level * 150:
                if map[x][y].name not in contract_targets:
                    if self.wealth > map[x][y].dungeon.base_level * 20:
                        ...
                        message('The Mayoress is taking action against the ' + map[x][y].name, libtcod.light_green, libtcod.dark_green)
    ...
    def rescue_corpse(self, hero):
        ...
        new_contract = Contract(asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, False)
        self.noticeboard.notices.append(new_contract)
...
class Contract:
    ...
    def __init__(self, asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, accepted):
...
class Dungeon:
    ...
    def gain_level(self):
        ...
        self.base_level += 1
        message('A Dungeon has increased in its evil power! Monsters flock from the forest!', libtcod.red, libtcod.dark_red)
...
class Hero:
    ...
    def __init__(self, inventory, equipment, wealth, base_level, reputation, stats, faction, x, y, itch, in_dungeon, name, age, hp, wounds, brain, alignment, order, activitylog, owner_name):
    ...
    def browse_stock(self, type = 'all'): #currently make an attempt to buy anything we like the look of. Can ask for a type.
        x = self.x
        y = self.y
            ...
            if x == shops[2]:
                if y == shops[3]: #shop location found
                    if shops[1] == 'Auction House':
                        ...
                        for index in range(len(map[x][y].shop.inventory)): #hero x,y matches the shop that he is present at, of course
                            item = map[x][y].shop.inventory[index]
                            appraised_value = appraise_value(item, self.faction, self.base_level) * self.brain.personality.markup #find out what we think it is worth
                            ...
                            if item_type == type or type == 'all': #this item matches our shopping preferences
                                ...
                                if item[5] <= appraised_value: #this item is priced in our range
                                    item_type = master_item_list[item[0]][9] #reference before we muck about with inventories
                                    price = item[5]
                                    if buy_item(self, map[x][y].shop, index): #make an attempt to buy it, if we think it is worthwhile. The player is a shop, in terms of the 'buy_item' function
                                        self.reputation.improve_reputation(map[x][y].faction)
                                        if map[x][y].faction == player_name:
                                            hero_message(self.name + ' bought ' + item_type + ' from ' + map[x][y].faction, libtcod.light_green, libtcod.dark_green)
                                        ...
                                        map[x][y].shop.gain_experience(item_experience(item)) #add experience to the shop equivalent to the level of the item
                                        self.inventory[-1][5] = appraise_value(self.inventory[-1], self.faction, self.base_level)
                                        if item_type == 'weapon' or item_type == 'scroll':
                                            self.activitylog['history'].append('I have bought a ' + item_type + ' from ' + map[x][y].faction + '.')
                                        else:
                                            self.activitylog['history'].append('I have bought some ' + item_type + ' from ' + map[x][y].faction + '.')
            ...
            if x == shops[2]:
                if y == shops[3]: #shop location found
                    ...
                    if no_items > 0:  #only if the number of items is greater than 1 for now
                        if type == 'all':
                            chosen_item_index = libtcod.random_get_int(0, 0, no_items) #select an item to sell (at the chosen price)
                            ...
                            if not map[x][y].shop == player:
                                hero_sale(x, y, map[x][y].shop, self, chosen_item_index) #(x, y, buy_shop, sell_shop, index)
                                self.reputation.improve_reputation(map[x][y].faction)
    ...
    def dress_thineself(self):
        ...
        item_list = []
        for index, item in enumerate(self.inventory):
            assessed_item_power = assess_item(item, self.faction, self.base_level) #a measure of how good the hero thinks the item is
        ...
        self.stats['perceived'] = self.base_level + self.stats['perceived_clothing'] + self.stats['perceived_weapon'] + self.stats['perceived_armor']
        ...
        if self.equipment['clothing']: #did we put anything on?
            clothing_bonus = self.equipment['clothing'][1][0] + self.equipment['clothing'][2][0] + master_item_list[self.equipment['clothing'][0]][3]
        else:
            clothing_bonus = 0

        if self.equipment['armor']:
            armor_bonus = self.equipment['armor'][1][0] + self.equipment['armor'][2][0] + master_item_list[self.equipment['armor'][0]][3]
        else:
            armor_bonus = 0

        if self.equipment['weapon']:
            weapon_bonus = self.equipment['weapon'][1][0] + self.equipment['weapon'][2][0] + master_item_list[self.equipment['weapon'][0]][3]
        else:
            weapon_bonus = 0
    ...
    def value_contract(self, contract):
        ...
        if contract_type == 'dungeon':
            ...
            if base_level: #so we have found the dungeon, it still exists
                ...
                message('A Hero has thrown away a void contract from the noticeboard.', libtcod.light_red, libtcod.dark_red)
                contract_void = 1
        ...
        if not contract_void:
            if asking_price >= base_value: #hero values this contract in the right region
                ...
                if log_information:
                    log_info('Contract acceptable at $' + str(asking_price) + ' by ' + self.name + ' but would have taken $' + str(base_value))
                    
                return True
            else:
                if log_information:
                    log_info('Contract not acceptable at $' + str(asking_price) + ' by ' + self.name + ', more like $' + str(base_value))
                return False
        else:
            return False
    ...
    def find_another_shop_social(self):
        ...
        for shop in shops:
            ...
            if shop[0] == 0:
                ...
                shop_weights[faction] = shop[0] #store the distances to the shops, for further weighting by additional factors.
    ...
    def find_another_shop_item(self, item_type = 'random'): #returns a sorted list of shops, by faction, and 'weighted' distance.
        ...
        for shop in shop_count_by_item_type(item_type):
            ...
            shop_weights[faction] -= shop[0] #adjust distances by the number of items held by the shops of the type we are after
    ...
    def resolve_contract(self):
                        ...
                        if len(self.activitylog['contract_cargo']) > 0:
                            if map[x][y].shop.wealth >= asking_price:
                                ...
                                if asking_faction == player_name:
                                    message(self.name + ' returns with loot!', libtcod.light_yellow, libtcod.dark_yellow)
                            else:
                                for item in self.activitylog['contract_cargo']:
                                    self.inventory.append(item)
                                    self.reputation.worsen_reputation(asking_faction) #not that bothered ... they get to keep the item ...
                        else:
                            if asking_faction == player_name:
        ...
        try: #whatever the result of the above, we should wrap things up and move on.
            the_mayoress.noticeboard.notices.remove(self.brain.contract) #may not have ever appeared here if self-motivated
        except:
    ...
    def populate_action_list(self): #function that tells us what to do, based on a chosen contract
        ...
        if debug_mode:
            message(self.name + ' POPULATE ACTIONS for: ' + str(mission_type) + ',' + str(contract_type) + ',' + str(asking_price), libtcod.light_grey, libtcod.light_grey)
        
        self.brain.decisions.append('resolve_contract') #every new contract has to end with the resolution of it afterwards.
        ...
        if contract_type == 'dungeon':
            ...
            if debug_mode:
                message(self.name + 'DELVING; PCVD PWR:' + str(self.stats['perceived']) + ', RL PWR:' + str(self.stats['real']) + ' DNGR:' + str(danger), libtcod.light_grey, libtcod.light_grey)
        ...
        if action:
            if not self.in_dungeon: #WE ARE IN TOWN!!!
                if action == 'move_to_monster':#is this redundant? You cannae duel without finding a monster? Entry action might be useful, though ...
                    ...
                    if self.move_to_monster(self.brain.contract.target_name): #code allows for the possibility of monster moving, or a break in the duel process
                        for monster in monster_list:
                    ...
                    if heroes_here:
                        ...
                        if log_or_not == 1:
                            self.activitylog['history'].append(pick_random_from_list(event_chat_with_pre) + hero_chat.name)
                        elif log_or_not == 2:
                            self.activitylog['history'].append(hero_chat.name + pick_random_from_list(event_chat_with_post))
                        else:
                            pass
                        self.brain.decisions.pop(-1)
                    ...
                    if map[x][y].shop:
                        self.browse_stock(self.brain.contract.target_name) #need a better function to assess whether the item is better than the one we hold?
                    self.brain.decisions.pop(-1)
                elif action == 'sell':
                    if map[x][y].shop:
                        ...
                        for corpse in dead_heroes:
                            ...
                            if [self.x, self.y] == [hero.x, hero.y]: #check if there is a hero here
                                if corpse[1] < hero_rot_time: #the corpse is still there
                                    ...
                                    self.activitylog['history'].append('I have restored the life force to a soul in need.')
                                    self.brain.decisions.pop(-1)
                                    
                                    hero_message(str(self.name) + ' has raised ' + str(hero.name) + '!', libtcod.light_yellow, libtcod.dark_yellow)
                                    if hero.faction == 'Necromancers':
                                        hero.activitylog['history'].append(self.name + ' has brought me back from death.')
                                    else:
                        ...
                        for corpse in dead_heroes:
                            ...
                            if [self.x, self.y] == [hero.x, hero.y]:
                                if corpse[1] < hero_rot_time: #the corpse is still there
                                    ...
                                    self.activitylog['history'].append('I have gained power from the corpse of another.')
                                    self.gain_experience(10)
                                    self.brain.decisions.pop(-1)
                                    ...
                                    birth_hero(gender)
                                    hero_message(hero_list[-1].name + ' takes up the fight!', libtcod.light_yellow, libtcod.dark_yellow)
                    
                elif action == 'find_corpse':
                    corpse_location = self.find_corpse(self.brain.contract.target_name)
                    if debug_mode:
                        message('Trying to find CORPSE: X - ' + str(corpse_location[0]) + ', Y - ' + str(corpse_location[1]), libtcod.light_grey, libtcod.light_grey)
                    if corpse_location[2]: #hero in a dungeon
                        for n in range(corpse_location[2]):
                            ...
                            self.brain.decisions.append('descend')
                        self.brain.contract.target_name = map[corpse_location[0]][corpse_location[1]].name #reassign target name from corpse, to dungeon
                ...
                if action == 'descend': #take on an aggressive manner
                    ...
                    if 'healer' in self.brain.personality.perk:
                        for corpse in dead_heroes:
                            hero = corpse[0]
                            if [self.x, self.y, self.in_dungeon] == [hero.x, hero.y, hero.in_dungeon]: #check if there is a hero here
                                if corpse[1] < hero_rot_time: #the corpse is still there
                                    hero.hp['current'] = hero.hp['max'] #ressurect! To max points, otherwise they just die again ...
                                    dead_heroes.remove(corpse)
                                    town_heroes.append(corpse[0])
    ...
    def check_cursed(self, status): #returns a list of heroes equipment, and whether this is cursed / blessed / uncursed (by request)
    ...
    def explore_floor(self):
        ...
        if local_feature == 'mundane':
            ...
            the_mayoress.noticeboard.rumours.append(Rumour(map[x][y].name, dungeon_level, 'treasure_room', rumour_length))
        elif local_feature == 'cursed_altar':
            self.brain.decisions.append('cursed_altar')
            rumour_length = 10 #days
            the_mayoress.noticeboard.rumours.append(Rumour(map[x][y].name, dungeon_level, 'cursed_altar', rumour_length))
    ...
    def self_motivate(self, motive): ##NEEDS A BIT MORE CREATIVE THOUGHT ON ADDITIONAL CONTRACTS ETC. PLUS PETITIONS
        ...
        if motive == 'wealth':
            if dice_roll < 65:
                ...
                if item_types_held:
                    new_contract = Contract(0, 'sell', pick_random_from_list(item_types_held), 'town', self.faction, [], False, True, True)
                    return new_contract
                else:
                    return False
            else:
            ...
            if map[x][y].shop:
                if dice_roll < 75:
                    ...
                    if map[x][y].shop != player:
                        message(self.name + ' is petitioning ' + map[x][y].faction + ' to bring forth quests!', libtcod.chartreuse, libtcod.dark_chartreuse)
    ...
    def hero_decision(self): #massively and gloriously deprecated. Never to be seen again.
                ...
                if not self.faction == 'Merchants': #we are an adventurer
                    if buy_motive == 3:
                        ...
                        if cash_motive >= 90:
                            ...
                            self.personality.motivation = ['buy', equipment_wants] #given shopping list. Hero will try and buy this stuff.
                            ...
                            self.personality.motivation = ['sell_scrolls', scrolls_held] #this number should correlate to attempts, not necessarily successful sales
                        else:
                            self.personality.motivation = ['leave_town']
        ...
        #event_item_sold 
                    ...
                    if self.activitylog['visit_history'][-1] == target_name: #JUST been in to the dungeon, i.e first active turn in town
            ...
            if self.hp['current'] > 0:
                if map[self.x][self.y].dungeon:
                    ...
                    if self.in_dungeon: #first things first, run a check for healers and defilers.
                        if libtcod.random_get_int(0,0,4) == 4: #one in four chance of getting to interact with a possible corpse
                            for corpse in dead_heroes:
                                hero = corpse[0]
                                if [self.x, self.y] == [hero.x, hero.y]: #check if there is a hero here
                    ...
                    event_chance = libtcod.random_get_int(0, 0, 100) #an event might take us, whether we like it or not.
    ...
    def world_sell_item(self): #currently abstracted to any item
        ...
        if no_items > 0: #we actually have something to sell
            ...
            if item_type == 'weapon' or item_type == 'scroll':
                self.activitylog['history'].append('I have sold a ' + item_type + ' to a shop in ' + pick_random_from_list(town_names))
            else:
                self.activitylog['history'].append('I have sold some ' + item_type + ' to a shop in ' + pick_random_from_list(town_names))
    ...
    def world_buy_item(self):
        ...
        if item_type == 'weapon' or item_type == 'scroll':
            self.activitylog['history'].append('I have bought a ' + item_type + ' from a shop in ' + pick_random_from_list(town_names))
        else:
            self.activitylog['history'].append('I have bought some ' + item_type + ' from a shop in ' + pick_random_from_list(town_names))
    ...
    def enter_town(self): #move hero from global list to local list
        ...
        if self.faction == 'Merchants':
            ...
            if libtcod.random_get_int(0, 0, 100) < 67: #use this to weight the proportion of sell cargo vs buy cargo
                ...
                self.wealth += libtcod.random_get_int(0, 10 * self.base_level, 100 * self.base_level) #little extra cash ...
                for n in range(cargo):
            ...
            self.activitylog['history'].append('I enter the town, ' + pick_random_from_list(event_enter_town_by_faction[self.faction]))
    ...
    def leave_town(self):
        ...
        if self.hp['current'] > 0:
            ...
            self.activitylog['history'].append('I leave town, ' + pick_random_from_list(event_leave_town_by_faction[self.faction]))
            self.activitylog['visit_history'] = []
        
    def move_to_new_shop(self, faction): #picks by faction
        if faction == 'Random':
            faction_destination = libtcod.random_get_int(0, 0, len(shop_list_inc_player) - 1) #find a shop random index from the available
            ...
            if map[x][y].dungeon.inventory:
                ...
                if item_type == 'weapon' or item_type == 'scroll':
                    if libtcod.random_get_int(0, 0, 3) == 3:
                        ...
                        self.activitylog['history'].append('I find a ' + str(item_type) + pick_random_from_list(event_item_location))
                else:
                    if libtcod.random_get_int(0, 0, 3) == 3:
                        self.activitylog['history'].append('I find some ' + str(item_type) + '.')
                        self.activitylog['history'].append(pick_random_from_list(event_item_found))
                    else:
                        self.activitylog['history'].append('I find some ' + str(item_type) + pick_random_from_list(event_item_location))
    ...
    def fight(self, encounter):
        ...
        if dungeon_vanquished:
            hero_message(self.name + ' cleanses the ' + map[self.x][self.y].name + '!', libtcod.green, libtcod.dark_green)
    ...
    def die(self):
        ...
        if len(self.inventory) > 0:
            for items in self.inventory:
                dropped = items
                if map[x][y].dungeon:
                    map[x][y].dungeon.inventory.append(dropped) #give the dungeon your items! What about monster deaths in the field?
        ...
        if contract.asking_faction == player_name:
            message(self.name + ' has died, releasing them from the terms of your contract!', libtcod.light_red, libtcod.dark_red)
            ...
            if asking_faction == player_name:
                ...
                if discuss_contract(contract_index, self): #we accept the terms, try and pay
                    if mission_type == 'item_hunt':
                        ...
                        self.brain.contract = the_mayoress.noticeboard.notices[contract_index] #set the motivation for the hero to carry out the contract
                        ...
                        message('You no longer have the funds to honour the contract! Contract repealed.', libtcod.light_red, libtcod.dark_red)
                        try: #whatever the result of the above, we should wrap things up and move on.
                            the_mayoress.noticeboard.notices.remove(contract) #may not have ever appeared here if self-motivated
                        except:
                            pass #might want to put in something about self-fulfilment here
    ...
    def resolve_contract_deprecated(self):
        ...
        if map[x][y].faction == player_name:
            message('The Hero ' + self.name + ' has completed a contract on your behalf.', libtcod.light_yellow, libtcod.dark_yellow)
            if number_items:
                message( str(number_items) + ' items added to your shop stock!', libtcod.light_green, libtcod.dark_green)
            else:
                message( 'Unfortunately, ' + self.name + ' has returned empty-handed.', libtcod.light_red, libtcod.dark_red)
...
class Personality:
    ...
    def __init__(self, markup, motivation, perk, quirk, verbosity, seeks_glory, seeks_social, seeks_balance, seeks_loot, seeks_contracts, seeks_leadership, seeks_wealth, seeks_knowledge): #plenty to go at here, nothing at present ...
...
class Metrics:
    ...
    def return_purchases_by_item_type(self, item_type, days = 10):
        ...
        for faction in active_faction_list(): #iterate through existing factions
            ...
            purchases_item_count.append([relevant_purchases, faction]) #for each faction store the total purchases by item type, for sorting elsewhere
...
class Room:
    ...
    def __init__(self, type, base_level, resources, owner_name, progress, goal): #counters etc. for a generic functional room
...
def measure_world_evil(): #Is there a neater or more intuitive way to deal with these sorts of variables? Specific class rather than global variables?
    ...
    total_item_evil = ( ( item_evil * 100 )/ ( average_evil + item_evil ) ) #normalised value between 1 - 100 for the total item evil in town
    ...
    if log_information:
        log_info('The world is ' + str(total_item_evil) + ' percent good - average evil ' + str(average_evil) + ' world evil = ' + str(item_evil))
...
def measure_world_chaos():
    ...
    if log_information:
        log_info('The world is ' + str(total_item_chaos) + ' percent lawful - average order ' + str(average_chaos) + ' world order = ' + str(item_chaos))
...
def check_world_chaos():
    ...
    if chaos_counter > chaos_event_rate:
        ...
        if len(inactive_sites) > 0:
            ...
            message('New Dungeon! The ' + map[x][y].name + ' has arisen from the forest!', libtcod.red, libtcod.dark_red)
...
def build_ecosystems():
    ...
    for z in range(0, max_monster_level+1, 1): #do a loop for every level of creature there is (also giving us a key for that level)
        ...
        ecosystems[z] = population_distribution # so key 1:[[medium rarity, 100],[common,300],[rare,305]] or something ...
...
def commission_dungeon(x, y): #new dungeon, set currently to start up a new 'starter dungeon'
    ...
    starter_dungeon = Dungeon(generate_inventory(60, 75, 1, 4), libtcod.random_get_int(0 ,1000, 1600), 1, Reputation(0, blank_faction_relations()), libtcod.random_get_int(0, min_dungeon_pop, max_dungeon_pop), 45, 45, {}, dungeon_type)
...
def new_random_map():
    ...
    build( ah_x, ah_y, 'Merchants', 'Auction House', '$', Store(generate_inventory(13,24,1,5),1500,1,Reputation(0, blank_faction_relations()),auction_house_owner_name, {'history':[]}, empty_layout))
    ...
    thief_store = Store(basic_inventory(),3000,2,Reputation(0, blank_faction_relations()),'Kilian Derva', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Thieves', 'Thief Den', '?', thief_store)
    
    assassin_store = Store(basic_inventory(),10000,8,Reputation(0, blank_faction_relations()),'Styl Marck', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Assassins', 'The Black House', '+', assassin_store)
    
    swashbuckler_store = Store(basic_inventory(),1500,2,Reputation(0, blank_faction_relations()),'Gudrun Steiner', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Swashbucklers', 'The Mermaids Wish', 'v', swashbuckler_store)
    
    fighter_store = Store(basic_inventory(),1000,1,Reputation(0, blank_faction_relations()),'Red Dog Muldrake', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Fighters', 'Guild of Tactics', 'x', fighter_store)
    
    barbarian_store =  Store(basic_inventory(),250,2,Reputation(0, blank_faction_relations()),'Oldran', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Barbarians', 'Goatskin Teepee', '^', barbarian_store)
    
    summoner_store = Store(basic_inventory(),1150,3,Reputation(0, blank_faction_relations()),'Ha-grey', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Summoners', 'Altar of Hope', '}', summoner_store)
    
    mage_store =  Store(basic_inventory(),1100,4,Reputation(0, blank_faction_relations()),'Firo Gradion', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Mages Guild', 'Honoured Gate', 'z', mage_store)
    
    temple_store = Store(basic_inventory(),1300,3,Reputation(0, blank_faction_relations()),'Piotr Serviz', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Temple', 'The Temple of Fred', '+', temple_store)
    
    church_store = Store(basic_inventory(),1200,2,Reputation(0, blank_faction_relations()),'Min Bo Lim', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Church', 'Unity Church', '+', church_store)
    
    druid_store = Store(basic_inventory(),100,1,Reputation(0, blank_faction_relations()),'Fire-Tooth', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
        build( location[0], location[1], 'Druids', 'Ever Glade', '#', druid_store)
    
    necromancer_store = Store(basic_inventory(),600,2,Reputation(0, blank_faction_relations()),'Herbert West', {'history':[]}, empty_layout)
    location = extract_random_from_list(foundations)
    if location:
...
def birth_hero(gender='m', level = 1, faction='Random'):
    ...
    new_hero = Hero(
        generate_inventory(1, 3 + level, 1, 2 + level, faction), #inventory, from 1 to 3 items, level 1 to 3, faction related.
        ...
        {'current':hero_base_hp + (level * hero_base_hp_level_gain), 'max':hero_base_hp + (level * hero_base_hp_level_gain)}, #hp - current / max
        [], #wounds,
        new_brain, #brain
        50, #alignment,
        50, #order
...
def spawn_monster(x, y, base_level):
    ...
    if len(monster_list) < maximum_monsters:
        ...
        new_monster = Hero(
            ...
            [], #wounds,
            new_brain, #personality, limited at the minute ... should randomise this for cool monster behaviour, tie name in to the personality
            45, #alignment,
            45, #order
            {'history' : [], 'kills' : [], 'visit_history': [], 'contract_cargo': [] }, #activitylog
            None #owner_name HACK SORRY
...
def generate_inventory(min_items, max_items, min_level, max_level, faction = 'all'): #note not enough items to reliably generate faction specific items yet
...
def create_item(level_low, level_hi, faction, type = 'all', alignment = 'all', order = 'all'): #send level bounds and faction to this function
        ...
        if master_item_list[index][3] >= level_low and master_item_list[index][3] <= level_hi:
            if faction == 'all' or faction in master_item_list[index][6]: #faction match
                if type == 'all' or type in master_item_list[index][9]: #type match
                    ...
                    item = [index, [quality, 0], [bonus, 0], [master_item_list[index][7], 0], master_item_list[index][2] + value_mod, 0, cursed]
                    # 0_index, 1_quality[real, known], 2_bonus[real,known], 3_effects[rl, kn], 4_realvalue, 5_marketvalue, 6_cursedstatus
                    item_got = 1
    return item
...
def appraise_value(item, faction, level): #item, faction of item appraiser, and level of item appraiser, ostensibly for NPC shopkeepers
...
def pass_turn():
    ...
    if day > seasons[0][1]: #Monthly cycle: Taxes / Relationships - Monthly status update? Store financial information with each transaction?
        ...
        if known_name == real_name: #we have already passed the first stage
            ...
            if known_level == real_level: #And the second, previously
                ...
                if known_faction == real_faction: #we already know everything about the item anyway ...
                    message('We know all there is to deduce, by investigation alone.', libtcod.light_grey, libtcod.light_blue)
                    ...
                    message('Excellent. We now know who tends to be associated with the ' + player_item_display(item), libtcod.light_grey, libtcod.light_blue)
                ...
                message('We now know the alignment, order and the level of the item.', libtcod.light_grey, libtcod.light_blue)
            ...
            if real_type == 'scroll':
                message('Ok, this ' + known_name + ' is actually a scroll of ' + real_name + '.', libtcod.light_grey, libtcod.light_blue)
            else:
                message('Ok, this ' + known_name + ' is actually a ' + real_name + '.', libtcod.light_grey, libtcod.light_blue)
            message(real_description, libtcod.light_grey, libtcod.light_blue)
            player_item_knowledge_list[item_index][0] = real_name #these aren't reassigning the original information ...
...
def auction_resolve():
    ...
    for m in range(len(bidders)):
        ...
        if appraise_confidence(auction_item, bidder_faction, bidder_level): #appraise_confidence(item, bidder_faction, bidder_level)
    ...
    if player_bid: #the player has put in a bid.
        if bid_count == 0: #no-one else has bid
            ...
            if auction_buy_item(buy_shop, sell_shop, 0, player_bid): #index is the item sitting in the sell shop, zero for all auction house activity so far
                message('You won an item at auction, for $' + str(player_bid) + ', submitting the only bid', libtcod.light_green, libtcod.dark_green)
                item_bought = 1
                ...
                message('You won the auction today with the only bid, but you can not afford it. Item fed to Dragons.', libtcod.light_red, libtcod.dark_red)
                item_bought = 1
            ...
            for confirm_win in range(len(bids_in_order)): #iterate through the bids, and give the item to the first valid bid

                win_faction = bids_in_order[confirm_win][1]
                win_price = bids_in_order[confirm_win][0]

                if not find_shop_by_faction(win_faction): #the faction doesn't have representation on the map, it returns false
                    map[x][y].shop.inventory.pop(0) #no bids on the item. Remove the item from AH inventory, lost to the ether.
                    message(str(win_faction) + ' wins an item at auction, for $'+ str(win_price) + ' and takes it out of town.', libtcod.light_yellow, libtcod.dark_yellow)
                    ...
                    if auction_buy_item(buy_shop, sell_shop, 0, win_price): #can they make the transaction?
                        if win_faction == player_name:
                            message('You won an item at auction, for $' + str(player_bid) + ', beating the competition', libtcod.light_green, libtcod.dark_green)
                            log_purchase(player_item_display(auction_item), player_bid, 'by Auction')
                            sales_metrics.log_item_bought(real_item[9], win_price, player_name, day_count)
                        else:
                            message(str(win_faction) + ' won an item at auction, with a winning bid of $' + str(win_price), libtcod.light_yellow, libtcod.dark_yellow)
        ...
        if bid_count == 0: #neither did anyone else ...
            map[x][y].shop.inventory.pop(0) #no bids on the item. Remove the item from AH inventory, lost to the ether.
            message('An Auction closes, and the item is not won. It is fed to the Dragons.', libtcod.light_red, libtcod.dark_red)
            item_bought = 1
            ...
            for confirm_win in range(len(bids_in_order)): #iterate through the bids, and give the item to the first valid bid
                ...
                if not find_shop_by_faction(win_faction): #the faction doesn't have representation on the map, it returns false
                    ...
                    sell_shop = map[x][y].shop #auction house
                    if auction_buy_item(buy_shop, sell_shop, 0, win_price): #can they make the transaction?
                        message(str(win_faction) + ' won an item at auction, with a winning bid of $' + str(win_price), libtcod.light_green, libtcod.dark_green)
                        sales_metrics.log_item_bought(real_item[9], win_price, win_faction, day_count)
                        item_bought = 1
                        break #stop iterating through bids, item has been won
...
def hi_score():
    ...
    libtcod.console_blit(hiscore_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.4, 0.4)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        libtcod.console_print_left(hiscore_popup, 1, 1, libtcod.BKGND_NONE, '1 0 0    H E R O E S   -   H I   S C O R E ! ! !   press enter')
        ...
        libtcod.console_print_left(hiscore_popup, 25, 6, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(item_sold_value))
        libtcod.console_print_left(hiscore_popup, 1, 7, libtcod.BKGND_NONE, 'NO. ITEMS BOUGHT: ' + str(no_items_bought))
        libtcod.console_print_left(hiscore_popup, 25, 7, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(item_bought_value))
        libtcod.console_print_left(hiscore_popup, 1, 8, libtcod.BKGND_NONE, 'NO. CONTRACTS LET: ' + str(no_contracts_let))
        libtcod.console_print_left(hiscore_popup, 25, 8, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(contract_value))
        ...
        libtcod.console_print_left(hiscore_popup, 5, 10, libtcod.BKGND_NONE, 'STOPPED TRADING ON: ' + datestamp + ' ' + season)
        ...
        for n in range(number_entries):
            ...
            libtcod.console_print_left(hiscore_popup, 2 + n, (n*9) + 14, libtcod.BKGND_NONE, 'HIGHSCORE #' + str(n + 1) + ' - ' + x_player)
            libtcod.console_print_left(hiscore_popup, 2 + n, (n*9) + 15, libtcod.BKGND_NONE, 'NO. ITEMS SOLD: ' + str(x_no_items_sold))
            libtcod.console_print_left(hiscore_popup, 27 + n, (n*9) + 15, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(x_item_sold_value))
            libtcod.console_print_left(hiscore_popup, 2 + n, (n*9) + 16, libtcod.BKGND_NONE, 'NO. ITEMS BOUGHT: ' + str(x_no_items_bought))
            libtcod.console_print_left(hiscore_popup, 27 + n, (n*9) + 16, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(x_item_bought_value))
            libtcod.console_print_left(hiscore_popup, 2 + n, (n*9) + 17, libtcod.BKGND_NONE, 'NO. CONTRACTS LET: ' + str(x_no_contracts_let))
            libtcod.console_print_left(hiscore_popup, 27 + n, (n*9) + 17, libtcod.BKGND_NONE, 'AT A COMBINED VALUE OF $' + str(x_contract_value))
            libtcod.console_print_left(hiscore_popup, 4 + n, (n*9) + 18, libtcod.BKGND_NONE, 'TAXES PAID:' + str(x_taxes_paid))
            libtcod.console_print_left(hiscore_popup, 6 + n, (n*9) + 19, libtcod.BKGND_NONE, 'STOPPED TRADING ON: ' + x_datestamp + ' ' + x_season)

            libtcod.console_print_left(hiscore_popup, 2 + n, (n*9) + 21, libtcod.BKGND_NONE, 'A score of:' + str(x_score))
...
def help():
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        libtcod.console_print_left(help_popup, 1, 1, libtcod.BKGND_NONE, '1 0 0    H E R O E S   -   H E L P   S C R E E N  /  K E Y   C O M M A N D S')

        for line in range(popup_width):
            libtcod.console_put_char(help_popup, line, 3, '=', libtcod.BKGND_NONE)

        libtcod.console_print_left(help_popup, 1, 5, libtcod.BKGND_NONE, 'Welcome to 100 Heroes, a Roguelike game for the IBM PC investigating the ')
        libtcod.console_print_left(help_popup, 1, 6, libtcod.BKGND_NONE, 'moral dilemmas of a practical capitalist in generic-fantasy-land.')
        libtcod.console_print_left(help_popup, 1, 8, libtcod.BKGND_NONE, 'You are the Player, a new boy in Retail in the town of <random_town_name>')
        libtcod.console_print_left(help_popup, 1, 9, libtcod.BKGND_NONE, 'and through a keen eye for a bargain, and better knowledge than the comp-')
        libtcod.console_print_left(help_popup, 1, 10, libtcod.BKGND_NONE, 'etition, can you improve turnover sufficiently before the Balrog attacks?')
        libtcod.console_print_left(help_popup, 1, 12, libtcod.BKGND_NONE, 'There are three parts to the screen:')
        libtcod.console_print_left(help_popup, 1, 13, libtcod.BKGND_NONE, '  a) The Town Plan, with surrounding countryside (on the left)')
        libtcod.console_print_left(help_popup, 1, 14, libtcod.BKGND_NONE, '  b) The Info Bar (on the right)')
        libtcod.console_print_left(help_popup, 1, 15, libtcod.BKGND_NONE, '  c) The message bars (on the bottom, and bottom right)')
        libtcod.console_print_left(help_popup, 1, 17, libtcod.BKGND_NONE, 'Start by mousing over the blue buildings in the Town Plan. These are shops.')
        libtcod.console_print_left(help_popup, 1, 18, libtcod.BKGND_NONE, 'Click on any shop to enter, although some people might not like you enough')
        libtcod.console_print_left(help_popup, 1, 19, libtcod.BKGND_NONE, 'just yet. Although with time, and an increase in your reputation, you will')
        libtcod.console_print_left(help_popup, 1, 20, libtcod.BKGND_NONE, 'soon have them banging down your door asking for a cup of sugar or an')
        libtcod.console_print_left(help_popup, 1, 21, libtcod.BKGND_NONE, 'emergency loan.')
        libtcod.console_print_left(help_popup, 1, 23, libtcod.BKGND_NONE, 'Find the Auction House, to place bids on todays Auction (see info bar) or')
        libtcod.console_print_left(help_popup, 1, 24, libtcod.BKGND_NONE, 'find your own shop (Player) to see your own stock. Interact with items by')
        libtcod.console_print_left(help_popup, 1, 25, libtcod.BKGND_NONE, 'pressing the number keys in shops, you can buy from other people and examine')
        libtcod.console_print_left(help_popup, 1, 26, libtcod.BKGND_NONE, 'your own stock. Your shop is a bright yellow.')
        libtcod.console_print_left(help_popup, 1, 28, libtcod.BKGND_NONE, 'The current status of the world (order-chaos, good-evil) is shown by the')
        libtcod.console_print_left(help_popup, 1, 29, libtcod.BKGND_NONE, 'two bars within the info bar. A more chaotic world means more dungeons.')
        libtcod.console_print_left(help_popup, 1, 31, libtcod.BKGND_NONE, 'Most keys should hopefully be clear in each screen, but there are a number')
        libtcod.console_print_left(help_popup, 1, 32, libtcod.BKGND_NONE, 'of important ones only accessible from the Town Map:')
        libtcod.console_print_left(help_popup, 1, 34, libtcod.BKGND_NONE, '      l: Show Fiscal Ledger             h: Show Heroes (h again to cycle)')# l,L = 108, 76 h,H = 104, 72
        libtcod.console_print_left(help_popup, 1, 35, libtcod.BKGND_NONE, '      c: Show Contract information    1/H: Indiv. Town Hero Information')
        libtcod.console_print_left(help_popup, 1, 36, libtcod.BKGND_NONE, '      d: Show Dungeon Roster            ?: Help Screen') #d = 100, D = 68, ?=63
        libtcod.console_print_left(help_popup, 1, 37, libtcod.BKGND_NONE, '      5: Speed up time                  6: Slow down time')
        libtcod.console_print_left(help_popup, 1, 38, libtcod.BKGND_NONE, '(space): Pause / Unpause      (backspace): Save and exit Game')
        libtcod.console_print_left(help_popup, 1, 39, libtcod.BKGND_NONE, '      i: View your shop                 a: Show Auction House')
        libtcod.console_print_left(help_popup, 1, 40, libtcod.BKGND_NONE, 'Now have a play around, and wait for your first hero to come to town!')
        ...
        libtcod.console_print_left(help_popup, 1, 44, libtcod.BKGND_NONE, '  This game is in development, apologies for any bugs.')
...
def show_ledger():
    ...
    libtcod.console_blit(ledger_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.6, 0.6)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        for transaction in range(ledger_display):
            ...
            libtcod.console_print_left(ledger_popup, 36, 5 + transaction, libtcod.BKGND_NONE, '| ' + item[3]) #bought/sold
            libtcod.console_print_left(ledger_popup, 44, 5 + transaction, libtcod.BKGND_NONE, '| ' + item[4]) #in money
            ...
            libtcod.console_set_foreground_color(ledger_popup, libtcod.dark_gray)
            libtcod.console_print_left(ledger_popup, 54, 5 + transaction, libtcod.BKGND_NONE, '| ' + item[6]) #other person / faction
...
def individual_hero_interface(index):
    ...
    libtcod.console_blit(indiv_hero_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        libtcod.console_print_left(indiv_hero_popup, 1, 5 , libtcod.BKGND_NONE, hero.name + ' of the ' + hero.faction + '.')
        ...
        if len(hero.inventory) > 0:
            ...
            libtcod.console_print_left(indiv_hero_popup, 2, 7 , libtcod.BKGND_NONE, 'Carrying ' + str(has_armor) + ' suit(s) of armor, '+ str(has_weapon) + ' weapon(s),')
            libtcod.console_print_left(indiv_hero_popup, 2, 8 , libtcod.BKGND_NONE, str(has_clothing) + ' item(s) of clothing and '+ str(scrolls_held) + ' scroll(s).')
        ...
        libtcod.console_set_foreground_color(indiv_hero_popup, libtcod.red)
        if hero.in_dungeon:
            libtcod.console_print_right(indiv_hero_popup, 58, 5 , libtcod.BKGND_NONE, 'IN DUNGEON lvl ' + str(hero.in_dungeon))
        else:
        ...
        for n in range(number_activity):
            libtcod.console_print_left(indiv_hero_popup, 2, 15 + number_activity - n , libtcod.BKGND_NONE, activitylist[n])
            libtcod.console_set_foreground_color(indiv_hero_popup, libtcod.red)
        
        if debug_mode:
            libtcod.console_set_foreground_color(indiv_hero_popup, libtcod.light_gray)
            if hero.brain.contract:
                libtcod.console_print_left(indiv_hero_popup, 40, 14 , libtcod.BKGND_NONE, "TGT:"+str(hero.brain.contract.target_name))
                libtcod.console_print_left(indiv_hero_popup, 40, 15 , libtcod.BKGND_NONE, "CTY:"+str(hero.brain.contract.contract_type))
                libtcod.console_print_left(indiv_hero_popup, 40, 16 , libtcod.BKGND_NONE, "MTY:"+str(hero.brain.contract.mission_type))
            m = len(hero.brain.decisions)
            for n in range(m):
                libtcod.console_print_left(indiv_hero_popup, 40, 17+n , libtcod.BKGND_NONE, "DEC:"+str(hero.brain.decisions[n]))
            
            perceived_power = hero.stats['perceived']
            real_power = hero.stats['real']
            
            libtcod.console_print_left(indiv_hero_popup, 5, 35 , libtcod.BKGND_NONE, "PERCEIVED POWER: " + str(perceived_power))
...
def hero_interface():
    ...
    libtcod.console_blit(hero_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        if hero_mode == 1:
            ...
            for hero_view in range(hero_display):
                ...
                libtcod.console_print_left(hero_popup, 1, (hero_view * 3)+5 , libtcod.BKGND_NONE, hero.name + ' of the ' + hero.faction + '.')
                libtcod.console_set_foreground_color(hero_popup, libtcod.dark_green)
                libtcod.console_print_left(hero_popup, 1, (hero_view * 3)+6 , libtcod.BKGND_NONE, 'LVL' + str(hero.base_level))
                ...
                libtcod.console_print_left(hero_popup, 7, (hero_view * 3)+6 , libtcod.BKGND_NONE, 'Carrying ' + str(items_held) + ' items and $' + str(hero.wealth) + ', with ' + str(len(hero.activitylog['kills'])) + ' kills.')
                ...
                if hero.in_dungeon:
                    libtcod.console_set_foreground_color(hero_popup, libtcod.red)
                    libtcod.console_print_right(hero_popup, 58, (hero_view * 3)+5 , libtcod.BKGND_NONE, 'IN DUNGEON lvl ' + str(hero.in_dungeon))
                elif not hero.in_dungeon:
                    libtcod.console_set_foreground_color(hero_popup, libtcod.yellow)
                    libtcod.console_print_right(hero_popup, 58, (hero_view * 3)+5 , libtcod.BKGND_NONE, motive_text + ' IN TOWN')
            ...
            for hero_view in range(hero_display):
                ...
                libtcod.console_print_left(hero_popup, 1, (hero_view * 10)+5 , libtcod.BKGND_NONE, hero.name + ' of the ' + hero.faction + '.')
                libtcod.console_set_foreground_color(hero_popup, libtcod.dark_green)
                libtcod.console_print_left(hero_popup, 1, (hero_view * 10)+6 , libtcod.BKGND_NONE, 'LVL' + str(hero.base_level))
                libtcod.console_print_right(hero_popup, 58, (hero_view * 10)+6 , libtcod.BKGND_NONE, map[hero.x][hero.y].name)
                libtcod.console_set_foreground_color(hero_popup, libtcod.green)
                libtcod.console_print_left(hero_popup, 4, (hero_view * 10)+6 , libtcod.BKGND_NONE, str(hero.base_level))
                libtcod.console_set_foreground_color(hero_popup, hero_health[1])
                ...
                if len(hero.inventory) > 0:
                    ...
                    libtcod.console_print_left(hero_popup, 2, (hero_view * 10)+7 , libtcod.BKGND_NONE, 'Carrying ' + str(has_armor) + ' suit(s) of armor, '+ str(has_weapon) + ' weapon(s),')
                    libtcod.console_print_left(hero_popup, 2, (hero_view * 10)+8 , libtcod.BKGND_NONE, str(has_clothing) + ' item(s) of clothing and '+ str(scrolls_held) + ' scroll(s).')
                ...
                libtcod.console_print_left(hero_popup, 36, (hero_view * 10)+9 , libtcod.BKGND_NONE, 'Has recently killed:')
                ...
                libtcod.console_set_foreground_color(hero_popup, libtcod.red)
                libtcod.console_print_right(hero_popup, 58, (hero_view * 10)+5 , libtcod.BKGND_NONE, 'IN DUNGEON lvl ' + str(hero.in_dungeon))
                ...
                for n in range(number_kills):
                    libtcod.console_print_left(hero_popup, 36, (hero_view * 10) + 10 + n , libtcod.BKGND_NONE, murderlist[n])
            ...
            for hero_view in range(hero_display):
                ...
                libtcod.console_print_left(hero_popup, 1, hero_view + 5 , libtcod.BKGND_NONE, hero.name + ' of the ' + hero.faction + '.')
                libtcod.console_set_foreground_color(hero_popup, levelcolor)
                libtcod.console_print_right(hero_popup, 58, hero_view + 5 , libtcod.BKGND_NONE, 'LVL' + str(hero.base_level))
...
def contract_interface():
    ...
    libtcod.console_blit(contract_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        for contract_view in range(contract_display):
            ...
            if contract_type == 'dungeon':
                if mission_type == 'default':
                    libtcod.console_print_left(contract_popup, 1, (contract_view * 3)+5 , libtcod.BKGND_NONE, 'Dungeon Delve')
                elif mission_type == 'item_hunt':
                    libtcod.console_print_left(contract_popup, 1, (contract_view * 3)+5 , libtcod.BKGND_NONE, 'Item Hunt    ')
            elif contract_type == 'monster':
                libtcod.console_print_left(contract_popup, 1, (contract_view * 3)+5 , libtcod.BKGND_NONE, 'Monster Hunt ')
            elif mission_type == 'raise_dead':
                libtcod.console_print_left(contract_popup, 1, (contract_view * 3)+5 , libtcod.BKGND_NONE, 'Rescue Corpse')
            ...
            libtcod.console_print_left(contract_popup, 2 + len(target_name), (contract_view * 3)+6 , libtcod.BKGND_NONE, '$' + str(asking_price))
            ...
            if assigned:
                ...
                libtcod.console_print_right(contract_popup, 58, (contract_view * 3)+6 , libtcod.BKGND_NONE, 'not assigned')

            libtcod.console_set_foreground_color(contract_popup, libtcod.yellow)
            libtcod.console_print_right(contract_popup, 58, (contract_view * 3)+5 , libtcod.BKGND_NONE, 'offered by ' + asking_faction)
...
def create_contract_interface():
    ...
    libtcod.console_blit(contract_bid_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.5, 0.5)
    libtcod.console_flush()
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        libtcod.console_print_left(contract_bid_popup, 5, 4, libtcod.BKGND_NONE, '$' + str(contract_price) + '            ')
        ...
        if contract_type == 'dungeon':
            ...
            if number_dungeons == 0:
                ...
                libtcod.console_print_left(contract_bid_popup, 1, 13, libtcod.BKGND_NONE, '(' + str(dungeon_list[dungeon_index][0]) + ')' + str(dungeon_list[dungeon_index][1]) )
            ...
            if number_monsters == 0:
                ...
                libtcod.console_set_foreground_color(contract_bid_popup, libtcod.light_red)
                libtcod.console_print_left(contract_bid_popup, 1, 13, libtcod.BKGND_NONE, '                        ')
                libtcod.console_print_left(contract_bid_popup, 1, 13, libtcod.BKGND_NONE, '(' + str(monster_list[monster_index].base_level) + ')' + monster_list[monster_index].name )
    ...
    if contract_type == 'monster':
        if player.wealth < contract_price:
            message('Insufficient funds to honour the contract! Contract not offered.', libtcod.light_red, libtcod.dark_red)
            ...
            generate_contract(contract_price, 'monster_hunt', target_name, 'monster', player_name, ['glory', 'contracts'], True, False, False)
            message('Contract generated. Offering $' + str(contract_price) + ' for the head of ' + monster_list[monster_index].name + '!', libtcod.light_yellow, libtcod.dark_yellow)
    
    elif contract_type == 'dungeon':
        if number_dungeons == 0:
            message('There are no dungeons present in the region! Contract not offered.', libtcod.light_red, libtcod.dark_red)
        else:
            target_name = dungeon_list[dungeon_index][1]
            generate_contract(contract_price, 'default', target_name, 'dungeon', player_name, ['glory', 'leadership', 'contracts'], True, False, False)
            message('Contract generated. Offering $' + str(contract_price) + ' for assaulting the ' + dungeon_list[dungeon_index][1] + '!', libtcod.light_yellow, libtcod.dark_yellow)
        ...
        if number_dungeons == 0:
            ...
            target_name = dungeon_list[dungeon_index][1]
            generate_contract(contract_price, 'item_hunt', target_name, 'dungeon', player_name, ['loot'], True, False, False)
            message('Contract generated. Offering $' + str(contract_price) + ' for plunder from the ' + dungeon_list[dungeon_index][1] + '!', libtcod.light_yellow, libtcod.dark_yellow)
...
def discuss_contract(contract_index, hero):
    ...
    libtcod.console_blit(discuss_contract_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.5, 0.5)
    libtcod.console_flush()
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        #print result of change in contract
        libtcod.console_blit(discuss_contract_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset, popup_yoffset, 1)
        libtcod.console_flush()
        #wait for key to modify bid, or submit
        key = libtcod.console_wait_for_keypress(True)
...
def generate_contract(asking_price, mission_type, target_name, contract_type, asking_faction, defining_motive, visible, paid, accepted):
...
def dungeon_interface():
    ...
    libtcod.console_blit(dungeon_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.5, 0.5)
    ...
    while not key.vk == libtcod.KEY_ENTER:
        ...
        for dungeon_view in range(dungeon_display):
            ...
            libtcod.console_print_left(dungeon_popup, 1, (dungeon_view * 3)+6 , libtcod.BKGND_NONE, 'LVL' + str(dungeon_map.dungeon.base_level))
            libtcod.console_set_foreground_color(dungeon_popup, libtcod.dark_yellow)
            libtcod.console_print_left(dungeon_popup, 10, (dungeon_view * 3)+6 , libtcod.BKGND_NONE, '$' + str(dungeon_map.dungeon.wealth))
            libtcod.console_set_foreground_color(dungeon_popup, libtcod.dark_yellow)
            libtcod.console_print_left(dungeon_popup, 17, (dungeon_view * 3)+6 , libtcod.BKGND_NONE,  str(len(dungeon_map.dungeon.inventory)) + ' items')
            libtcod.console_set_foreground_color(dungeon_popup, libtcod.dark_yellow)
            libtcod.console_print_left(dungeon_popup, 30, (dungeon_view * 3)+6 , libtcod.BKGND_NONE,  str(dungeon_map.dungeon.population) + ' monsters')
...
def player_item_edit_value(index):
    ...
    libtcod.console_blit(value_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)
    ...
    message('Item value changed for ' + player_item_display(player.inventory[index]), libtcod.light_yellow, libtcod.dark_yellow)
...
def item_interact(x, y, index):
    ...
    if index < testlength:
        ...
        libtcod.console_blit(popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)
        ...
        libtcod.console_set_foreground_color(popup, libtcod.yellow)    #item display, consider changing colour based on item?
...
def hero_sale(x, y, buy_shop, sell_shop, index): #hero trying to palm off their wares, index given as the hero inventory index
    ...
    if faction == player_name: #are we dealing with the player?
        ...
        if index < testlength: #is there an item available?
            ...
            while not resolved:
                ...
                for clearx in range(popup_width): #draw a shadow behind the popup
                    for cleary in range(popup_height):
                        libtcod.console_set_back(herosale_popup, clearx, cleary, libtcod.Color(0, 0, 0), libtcod.BKGND_SET )
                libtcod.console_blit(herosale_popup, 0, 0, popup_width, popup_height, 0, popup_xoffset - 1, popup_yoffset - 1, 0.75, 0.75)

                for clearx in range(popup_width): #draw a box, clear text
                    for cleary in range(popup_height):
                        libtcod.console_set_back(herosale_popup, clearx, cleary, libtcod.Color(15,35,5), libtcod.BKGND_SET )
                ...
                libtcod.console_print_left(herosale_popup, 7, 4, libtcod.BKGND_NONE, item_quality_display(item_known_quality))
                libtcod.console_print_left(herosale_popup, 7, 5, libtcod.BKGND_NONE, item_bonus_display(item_known_bonus))
                ...
                libtcod.console_set_foreground_color(herosale_popup, libtcod.yellow)    #item display, consider changing colour based on item?
                libtcod.console_print_left(herosale_popup, 2, 2, libtcod.BKGND_NONE, player_item_display(item) + ' for sale.')
                ...
                if key.c == 98: #b?
                    ...
                    if buy_item(buy_shop, sell_shop, index):
                        hero_message(sell_shop.name + ' sold ' + real_type + ' to Player!', libtcod.light_green, libtcod.dark_green)
                        player.gain_experience(item_experience(item))
                    ...
                    hero_message('You decline the offer from ' + sell_shop.name, libtcod.light_yellow, libtcod.dark_yellow)
                    player.gain_experience(1)
                    resolved = True
                else:
                    pass
...
def auction_buy_item(buy_shop, sell_shop, index, buy_price): #index is the item sitting in the sell_shop
    ...
    if buy_shop.wealth >= buy_price:
        ...
        new_item_value = int(buy_price * 1.2) #set the new value to match what it was bought for ... 1.2 value is the margin
...
def faction_house_interface(x, y):
    ...
    ##pop_up = 1
    shp = libtcod.console_new(VIEWPORT_WIDTH - 2, VIEWPORT_HEIGHT - 2) #shop interface screen, nearly as big as normal town view console
        ...
        while not key.vk == libtcod.KEY_ENTER:
            ...
            if not stock_level:
                ...
                for z in range(offset, no_items + offset):
                    ...
                    libtcod.console_set_foreground_color(shp, libtcod.white)
                    libtcod.console_print_left(shp, 1, 8+z - offset, libtcod.BKGND_NONE, str(z - offset) + " " + player_item_display(item))
                    libtcod.console_set_foreground_color(shp, libtcod.dark_yellow)
...
def show_heroes(heroes_present):
    ...
    for zzz in range(length):
        libtcod.console_set_foreground_color(inf, libtcod.light_blue)
        libtcod.console_print_left(inf, 16, 22 + zzz, libtcod.BKGND_NONE, heroes_present[zzz].name + ', ' + heroes_present[zzz].faction)
...
def auction_player_bid(item, reserve_price):
    ...
    message('Bid of $' + str(player_bid) + ' placed on ' + player_item_display(item), libtcod.light_yellow, libtcod.dark_yellow)
    #display the player bid in the message screen

def show_future_auctions(x, y):

    lot_console = libtcod.console_new(VIEWPORT_WIDTH - 2, VIEWPORT_HEIGHT - 2) #new console for showing the future lots
    inventory = map[x][y].shop.inventory
    ...
    if len(inventory):
        ...
        libtcod.console_print_left(lot_console, 1, 5, libtcod.BKGND_NONE, 'No upcoming auctions. Waiting for some people to die and bequeath.')
    ...
    if not upcoming:
        ...
        for z in range(no_lots):
            libtcod.console_print_left(lot_console, 1, 8+z, libtcod.BKGND_NONE, str(z) + ' ' + player_item_display(inventory[z]))
        if upcoming > no_lots:
        ...
        while not key.vk == libtcod.KEY_ENTER:
            ...
            for z in range(offset, no_lots + offset):
                libtcod.console_print_left(lot_console, 1, 8+z - offset, libtcod.BKGND_NONE, str(z) + ' ' + player_item_display(inventory[z]))
...
def show_bidders(x, y):
    ...
    bid_console = libtcod.console_new(VIEWPORT_WIDTH - 2, VIEWPORT_HEIGHT - 2) #shop interface screen, nearly as big as normal town view console
    ...
    if not interest_level:
        ...
        for z in range(no_bidders):
            libtcod.console_print_left(bid_console, 1, 8+z, libtcod.BKGND_NONE, str(z) + ' A visit from ' + bidders[z][0])
        ...
        while not key.vk == libtcod.KEY_ENTER:
            ...
            for z in range(offset, no_bidders + offset):
                libtcod.console_print_left(bid_console, 1, 8+z - offset, libtcod.BKGND_NONE, str(z) + ' A visit from ' + bidders[z][0])
...
def render_all():
    ...
    libtcod.console_blit(con_over, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, 0, 0.75, 0.0) #blitted after the main console, background clear
...
def hero_rot():
    ...
    for hero in dead_heroes:
        hero[1] += 1
        if hero[1] >= hero_rot_time: #should only happen once, but we keep a log of all dead heroes anyway, and of course respective time of death
...
def setup_new_data():
    ...
    message_buffer = [ blank_message, blank_message, blank_message, blank_message, blank_message, blank_message, blank_message, blank_message]
    hero_message_buffer = [ blank_message, blank_message, blank_message, blank_message, blank_message, blank_message, blank_message, blank_message]
...
def build_shop_and_dungeon_lists():
    ...
    for y in range(VIEWPORT_HEIGHT):
        for x in range(VIEWPORT_WIDTH):
            if map[x][y].faction == player_name:
                shop_list_inc_player.append( [ map[x][y].faction, map[x][y].name, x, y ] ) #add player details to this copy of the shop list
